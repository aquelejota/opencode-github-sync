...
export class GitAuthError extends Error {
    ...
    constructor(operation, result) {
        super(`${operation} failed: GitHub credentials are missing or lack access.\n` +
...
export class GitError extends Error {
    result;
    constructor(operation, result) {
        super(`${operation} failed: ${firstLine(result.stderr || result.stdout) || "unknown git error"}`);
        this.result = result;
        this.name = "GitError";
    }
}
/** Convert a failed `GitResult` into the most specific error we can. */
export function toGitError(operation, result) {
...
export function parsePorcelainPaths(output) {
    ...
    for (const line of String(output ?? "").split("\n")) {
        ...
        if (withoutStatus === line.trim() && !/^\s*[MADRCU?!]/.test(line))
            continue;
