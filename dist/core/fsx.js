...
export function rejectLink(target, role) {
    try {
        if (fs.lstatSync(target).isSymbolicLink())
            throw new SymlinkRefusedError(role, target);
    }
    catch (err) {
...
export function writeIfDiffers(filePath, content) {
...
export function copyFileIfExists(src, dst) {
...
export function mirrorFile(src, dst) {
    rejectLink(src, "source");
    rejectLink(dst, "target");
    if (!fs.existsSync(src)) {
...
export function copyDirRecursive(src, dst, options = {}) {
    const { excludeFiles = new Set(), excludeDirs = new Set() } = options;
    if (!fs.existsSync(src))
        return 0;
    rejectLink(src, "source");
    rejectLink(dst, "target");
    ...
    for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
        ...
        if (entry.isSymbolicLink())
            throw new SymlinkRefusedError("source", srcPath);
 ...
 */
export function syncDirIncremental(srcDir, dstDir, options = {}) {
    const { excludeFiles = new Set(), excludeDirs = new Set(), deleteExtraneous = true, } = options;
    ...
    const walkDestination = (rel) => {
        ...
        for (const entry of entries) {
            ...
            const dstChild = path.join(dstDir, childRel);
            if (entry.isSymbolicLink())
                throw new SymlinkRefusedError("target", dstChild);
            if (entry.isDirectory()) {
                if (excludeDirs.has(entry.name))
                    continue;
...
export function replaceFileAtomically(src, dst, options = {}) {
    rejectLink(src, "source");
    rejectLink(dst, "target");
    if (!fs.existsSync(src)) {
...
export function replaceDirAtomically(src, dst, options = {}) {
    rejectLink(src, "source");
    rejectLink(dst, "target");
    if (!fs.existsSync(src)) {
        removeRecursive(dst);
 ...
 * modified.
 */
export function removeNestedGitDirs(...dirs) {
    let removed = 0;
    const walk = (dir) => {
        let entries;
        try {
