...
import { copyDirRecursive, copyPath, ensureDir, mirrorFile, removeNestedGitDirs, removeRecursive, replaceDirAtomically, replaceFileAtomically, syncDirIncremental, writeIfDiffers, } from "./fsx.js";
import { git } from "./git.js";
import { parseJsonc } from "./jsonc.js";
import { configTextForRepo, findConfigFile } from "./overrides.js";
import { AGENTS_DIR, DATA_DIR, STATE_DIR, getRoots } from "./paths.js";
/**
...
export function createStageContext(repoRoot, settings, reporter, force, roots = getRoots()) {
    return { repoRoot, roots, settings, reporter, force };
}
function dataFilesFor(settings) {
...
export function stageIn(ctx) {
    ...
    if (!settings.includeCredentials) {
        for (const file of CREDENTIAL_FILES)
            removeRecursive(path.join(repoData, file));
    }
 ...
 */
function stageSkillLockIn(ctx, repoAgents) {
    ...
    if (merged)
        writeIfDiffers(repoLock, `${JSON.stringify(merged, null, 2)}\n`);
    else
        mirrorFile(localLock, repoLock);
}
...
function mergeSkillLocks(remote, local) {
    if (!remote || !local)
        return undefined;
    return {
        ...remote,
 ...
 */
export function normalizeConfigForRepo(ctx) {
...
function sanitizeRelative(entry) {
    return entry
        .replace(/^[A-Za-z]:/, "")
        .replace(/[\\/]+/g, "_")
        .replace(/^[._]+/, "");
}
function stageExtraPathsIn(ctx) {
    for (const { source, repo } of extraPathEntries(ctx)) {
        ...
        if (fs.statSync(source).isDirectory())
            syncDirIncremental(source, repo);
        else
            mirrorFile(source, repo);
    }
}
/** Copy everything from the repository worktree back onto the machine. */
export function stageOut(ctx) {
    ...
    for (const rel of DATA_PATHS) {
        ...
        if (fs.existsSync(source))
            copyPath(source, path.join(roots.data, rel));
    }
...
function stageSkillLockOut(ctx, repoAgents) {
    const repoLock = path.join(repoAgents, SKILL_LOCK);
    const localLock = path.join(path.dirname(ctx.roots.agents), SKILL_LOCK);
    ...
    if (merged)
        writeIfDiffers(localLock, `${JSON.stringify(merged, null, 2)}\n`);
    else
        replaceFileAtomically(repoLock, localLock);
}
function stageExtraPathsOut(ctx) {
    for (const { source, repo } of extraPathEntries(ctx)) {
        if (!fs.existsSync(repo))
            continue;
        if (fs.statSync(repo).isDirectory())
            replaceDirAtomically(repo, source);
        else
            replaceFileAtomically(repo, source);
    }
}
/**
 ...
 */
export function prunePluginCache(configRoot, cacheRoot) {
    const cachePackage = path.join(cacheRoot, "package.json");
    const configFile = findConfigFile(configRoot);
    if (!fs.existsSync(cachePackage) || !configFile)
        return 0;
    let config;
    try {
    ...
    const enabled = new Set((config?.plugin ?? []).map(pluginSpecToName));
 ...
 */
export function pluginSpecToName(spec) {
    if (spec.startsWith("@")) {
