...
function runInit(args, mode) {
    ...
    heading("Next");
    write(`  ${paint("1.", style.faint)} ${mode === "init" ? "opencode-sync push" : "opencode-sync pull"}`);
...
async function runSessions(args) {
    ...
    if (sub === "include" || sub === "exclude") {
        ...
        if (!id)
            throw new SyncError(`Usage: opencode-sync sessions ${sub} <session-id>`);
...
function runHelp() {
    ...
    write();
    rule();
    write(`  ${paint("docs", style.faint)}  ${paint("https://github.com/doomsday616/opencode-github-sync", style.muted)}`);
    write();
}
