...
import type { SyncAction, WorkerMessage } from "./progress.js";

const action: SyncAction = workerData?.action === "push" ? "push" : "pull";
...
function countShards(configRoot: string): number {
  try {
    return fs
...
async function run(): Promise<void> {
  ...
  const reporter: Reporter = {
    ...
    changes: (summary: ChangeSummary, files: ChangedFile[]) =>
      post({ type: "changes", action, count: files.length }),
  };

  try {
    const result = await withSyncLock(roots.config, { waitMs: 10_000 }, () =>
    ...
    post({ type: "error", action, message: (error as Error).message });
  }
}
