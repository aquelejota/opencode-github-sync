...
function isDerivedPath(file: string): boolean {
  const normalized = String(file ?? "").replace(/\\/g, "/");
  if (DERIVED_EXCEPTIONS.has(normalized)) return false;
 ...
 */
function resolveDerivedConflicts(ctx: Context): boolean {
  ...
  for (let round = 0; round < REBASE_RESOLVE_LIMIT; round++) {
    ...
    if (!files.every(isDerivedPath)) return false;
